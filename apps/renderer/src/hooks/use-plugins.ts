...
async function loadJsonPlugins(pluginsDir: string): Promise<Command[]> {
	...
	try {
		...
		for (const entry of entries) {
			...
			try {
				...
				commands.push(pluginToCommand(result.data))
			} catch (e) {
		...
		// Directory doesn't exist — no JSON plugins
	}
	return commands
}

async function loadScriptablePlugins(): Promise<Command[]> {
	const commands: Command[] = []
	try {
		...
		for (const plugin of scriptablePlugins) {
			for (const cmd of plugin.commands) {
				commands.push(scriptableCommandToCommand(plugin, cmd))
			}
		}
	} catch (e) {
...
export function usePlugins(): UsePluginsReturn {
	...
	const loadPlugins = useCallback(async () => {
		...
		try {
			...
			const tier1 = await loadJsonPlugins(pluginsDir)
			const tier2 = await loadScriptablePlugins()

			setPlugins([...tier1, ...tier2])
		} catch (e) {
